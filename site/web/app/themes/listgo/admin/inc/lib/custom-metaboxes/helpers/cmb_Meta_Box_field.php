...
//			: get_metadata( $type, $id, $field_id, ( $single || $repeat ) /* If multicheck this can be multiple values */ );
	...
	public function escaped_value( $func = 'esc_attr', $meta_value = '' ) {
		...
		if ( false === $cb || $this->escaping_exception() ) {
			...
			return $meta_value != '' ? $meta_value : $this->args( 'default' );
		} elseif ( $cb ) {
			// Ok, callback is good, let's run it.
			return call_user_func( $cb, $meta_value, $this->args(), $this );
		}
		...
		$meta_value = $meta_value != '' ? $meta_value : $this->args( 'default' );
	...
	public function render_field() {
		...
		print $this->args( 'before' );

		$this_type = new cmb_Meta_Box_types( $this );
		$this_type->render();

		print $this->args( 'after' );
	...
	public function _set_field_defaults( $args ) {
		...
		$args['options']    = 'group' == $args['type'] ? wp_parse_args( $args['options'], array(
			'add_button'    => esc_html__( 'Add Group', 'listgo' ),
			'remove_button' => esc_html__( 'Remove Group', 'listgo' ),
		) ) : $args['options'];
