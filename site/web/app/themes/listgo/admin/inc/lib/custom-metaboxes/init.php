...
class cmb_Meta_Box {
	...
	public function register_scripts() {
		...
		if ( 3.5 <= $wp_version ) {
			...
			$styles[] = 'spectrum';
			if ( !is_admin() ) {
				wp_enqueue_script('spectrum', WILOKE_AD_ASSET_URI . 'js/spectrum.js', array('jquery'), null, true);
            	wp_enqueue_style('spectrum', WILOKE_AD_SOURCE_URI . 'css/spectrum.css', array(), null);
			}
		} else {
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'up_arrow'        => '[ ↑ ]&nbsp;',
			'down_arrow'      => '&nbsp;[ ↓ ]',
			'check_toggle'    => esc_html__( 'Select / Deselect All', 'listgo' ),
		) ) );
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		{
			echo "\n<tr><td></td><td><p class='cmb_metabox_description'>".$meta_box['description']."</p></td></tr>\n";
		}
	...
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
		  ...
		  $cmb_url = str_replace(
				...
				get_template_directory() . '/admin/inc/lib/custom-metaboxes'
			);
		}
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, esc_html__( 'Save', 'listgo' ) );
