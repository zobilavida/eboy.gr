...
window.CMB = (function(window, document, $, undefined){
	...
	cmb.initDatePickers = function( $selector ) {
		...
		if ( $selector.data('date_format') !== '' ){
			$selector.datepicker({
				'dateFormat': $selector.data('date_format')
			});
		}else{
			$selector.datepicker();
		}
    ...
    $.fn.wilokeDependency = function()
    {
        ...
        $target.on('click change', function (event){
            ...
            if ( status )
            {
                ...
                {
                    $this.closest('tr, .wiloke__filter').removeClass('hidden');
                }
            }else{

                $this.closest('tr, .wiloke__filter').addClass('hidden');
            }
        });
