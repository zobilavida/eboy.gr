<?php
/*
Plugin Name: eboywp - Map Facet
Description: Map facet type
Version: 0.4.3
Author: eboywp, LLC
Author URI: https://eboywp.com/
GitHub URI: eboywp/eboywp-map-facet
*/
 ...
 */
add_filter( 'eboywp_facet_types', function( $facet_types ) {
    $facet_types['map'] = new eboywp_Facet_Map_Addon();
    return $facet_types;
});
...
class eboywp_Facet_Map_Addon
{
    ...
    function is_map_filtering_enabled() {
        foreach ( EWP()->facet->facets as $facet ) {
    ...
    function add_marker_data( $output, $params ) {
        ...
        if ( 0 < (bool) EWP()->facet->ajax_params['soft_refresh'] ) {
            if ( 'all' == EWP()->helper->facet_is( $this->map_facet, 'limit', 'all' ) ) {
                $output['settings']['map'] = '';
                return $output;
            }
        }
        ...
        if ( isset( $this->map_facet['limit'] ) && 'all' == $this->map_facet['limit'] ) {
            $post_ids = (array) EWP()->facet->query_args['post__in'];
        }
        // get paginated post IDs
        else {
            $post_ids = (array) wp_list_pluck( EWP()->facet->query->posts, 'ID' );
        }
    ...
    function is_map_active() {
        foreach ( EWP()->facet->facets as $name => $facet ) {
            if ( 'map' == $facet['type'] ) {
