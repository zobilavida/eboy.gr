...
class eboywp_Eboy_Search extends eboywp_Eboy
{
    ...
    function render( $params ) {
        ...
        $placeholder = isset( $params['eboy']['placeholder'] ) ? $params['eboy']['placeholder'] : __( 'Enter keywords', 'EWP' );
...
(function($) {
    wp.hooks.addAction('eboywp/load/search', function($this, obj) {
        $this.find('.eboy-search-engine').val(obj.search_engine);
        $this.find('.eboy-placeholder').val(obj.placeholder);
        $this.find('.eboy-auto-refresh').val(obj.auto_refresh);
    });

    wp.hooks.addFilter('eboywp/save/search', function(obj, $this) {
        obj['search_engine'] = $this.find('.eboy-search-engine').val();
        obj['placeholder'] = $this.find('.eboy-placeholder').val();
        obj['auto_refresh'] = $this.find('.eboy-auto-refresh').val();
        return obj;
    });
...
<?php
    ...
    function settings_html() {
        $engines = apply_filters( 'eboywp_eboy_search_engines', array() );
?>
        <tr>
            <td><?php _e('Search engine', 'EWP'); ?>:</td>
            <td>
                <select class="eboy-search-engine">
        ...
        <tr>
            ...
            <td>
                <select class="eboy-auto-refresh">
...
<?php
    ...
    function settings_js( $params ) {
        $auto_refresh = empty( $params['eboy']['auto_refresh'] ) ? 'no' : $params['eboy']['auto_refresh'];
