...
class eboywp_Indexer
{
    ...
    function __construct() {
        if ( apply_filters( 'eboywp_indexer_is_enabled', true ) ) {
            $this->run_hooks();
            $this->run_cron();
        }
    }
    ...
    function run_hooks() {
        ...
        add_action( 'eboywp_indexer_cron',     array( $this, 'get_progress' ) );
     ...
     */
    function run_cron() {
        if ( ! wp_next_scheduled( 'eboywp_indexer_cron' ) ) {
            wp_schedule_single_event( time() + 300, 'eboywp_indexer_cron' );
        }
    }
    ...
    function delete_post( $post_id ) {
        ...
        $wpdb->query( "DELETE FROM {$wpdb->prefix}eboywp_index WHERE post_id = $post_id" );
    }
    ...
    function index( $post_id = false ) {
        ...
        if ( false === $post_id ) {
            ...
            if ( 0 < $touch ) {
                ...
                // Clear table values
                $wpdb->query( "TRUNCATE TABLE {$wpdb->prefix}eboywp_index" );
            }
        ...
        $args = apply_filters( 'eboywp_indexer_query_args', $args );
        ...
        if ( 0 < $offset ) {
            $post_ids = json_decode( get_option( 'eboywp_indexing' ), true );
        }
        else {
            ...
            // Store post IDs
            if ( $this->index_all ) {
                update_option( 'eboywp_indexing', json_encode( $post_ids ), false );
            }
        }
        ...
        foreach ( $post_ids as $counter => $post_id ) {
            ...
            if ( $this->index_all ) {
                if ( 0 == ( $counter % $this->chunk_size ) ) {
                    ...
                    update_option( 'eboywp_transients', json_encode( $transients ) );
                }
            }
        ...
        // Indexing complete
        if ( $this->index_all ) {
            update_option( 'eboywp_transients', '' );
            update_option( 'eboywp_indexing', '' );
        }
    }
    ...
    function get_row_data( $defaults ) {
        ...
        if ( 'tax/' == substr( $source, 0, 4 ) ) {
            ...
            $hierarchy = EWP()->helper->get_term_depths( $taxonomy );
    ...
    function index_row( $params ) {
        ...
        $params = apply_filters( 'eboywp_index_row', $params, $this );
    ...
    function get_transient( $name = false ) {
        $transients = get_option( 'eboywp_transients' );
