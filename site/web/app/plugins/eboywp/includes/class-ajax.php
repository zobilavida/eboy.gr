...
class eboywp_Ajax
{
    ...
    /* (array) eboywp-related GET variables */
    public $url_vars = array();

    /* (boolean) EWP template shortcode? */
    public $is_shortcode = false;

    /* (boolean) Is a eboywp refresh? */
    ...
    function __construct() {
        ...
        if ( current_user_can( 'manage_options' ) ) {
            if ( check_ajax_referer( 'EWP_admin_nonce', 'nonce', false ) ) {
                add_action( 'wp_ajax_eboywp_save', array( $this, 'save_settings' ) );
                add_action( 'wp_ajax_eboywp_rebuild_index', array( $this, 'rebuild_index' ) );
                add_action( 'wp_ajax_eboywp_heartbeat', array( $this, 'heartbeat' ) );
                add_action( 'wp_ajax_eboywp_license', array( $this, 'license' ) );
                add_action( 'wp_ajax_eboywp_backup', array( $this, 'backup' ) );
            }
        }

        // Non-authenticated
        add_action( 'eboywp_refresh', array( $this, 'refresh' ) );
        add_action( 'wp_ajax_nopriv_eboywp_resume_index', array( $this, 'resume_index' ) );

        // Deprecated
        add_action( 'wp_ajax_eboywp_refresh', array( $this, 'refresh' ) );
        add_action( 'wp_ajax_nopriv_eboywp_refresh', array( $this, 'refresh' ) );
    ...
    function intercept_request() {
        ...
        if ( $this->is_preload ) {
            ...
            $this->url_vars = apply_filters( 'eboywp_preload_url_vars', $this->url_vars );
        }
    ...
    /**
     * Force eboywp to use the default WP query
     */
    function update_query_vars( $query ) {
        ...
        $is_main_query = apply_filters( 'eboywp_is_main_query', $is_main_query, $query );

        if ( $is_main_query ) {

            // Set the flag
            $query->set( 'eboywp', true );
    ...
    function inject_template() {
        ...
        if ( empty( $this->output['settings'] ) ) {
            $html = __( 'eboywp was unable to auto-detect the post listing', 'EWP' );
        }
    ...
    function resume_index() {
        $touch = (int) EWP()->indexer->get_transient( 'touch' );
    ...
    function refresh() {
        ...
        echo apply_filters( 'eboywp_ajax_response', $output, array(
            'data' => $data
        ) );
    ...
    function backup() {
            ...
            if ( ! empty( $items ) ) {
                foreach ( $items as $item ) {
                    ...
                    elseif ( 'template' == substr( $item, 0, 8 ) ) {
                        ...
                        $output['templates'][] = EWP()->helper->get_template_by_name( $item_name );
                    }
                }
            }
        ...
        elseif ( 'import' == $action_type ) {
            ...
            update_option( 'eboywp_settings', json_encode( $settings ) );

            if ( ! empty( $status['imported'] ) ) {
                echo ' [<strong>' . __( 'Imported', 'EWP' ) . '</strong>] ' . implode( ', ', $status['imported'] );
            }
            if ( ! empty( $status['skipped'] ) ) {
                echo ' [<strong>' . __( 'Skipped', 'EWP' ) . '</strong>] ' . implode( ', ', $status['skipped'] );
            }
        }
    ...
    function license() {
        ...
        if ( ! is_wp_error( $request ) || 200 == wp_remote_retrieve_response_code( $request ) ) {
            update_option( 'eboywp_license', $license );
            update_option( 'eboywp_activation', $request['body'] );
            echo $request['body'];
        }
        else {
            echo json_encode( array(
                'status'    => 'error',
                'message'   => __( 'Error', 'EWP' ) . ': ' . $request->get_error_message(),
            ) );
        }
