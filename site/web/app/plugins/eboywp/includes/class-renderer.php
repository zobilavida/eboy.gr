...
class eboywp_Renderer
{
    ...
    function render( $params ) {
        ...
        // Hook params
        $params = apply_filters( 'eboywp_render_params', $params );
        ...
        if ( 'wp' == $params['template'] ) {
            ...
            $query_args = EWP()->ajax->query_vars;
        }
        else {
            $this->template = EWP()->helper->get_template_by_name( $params['template'] );
        ...
        if ( empty( $this->query_args ) ) {
            ...
            $this->query_args = apply_filters( 'eboywp_query_args', $query_args, $this );
            ...
            $use_relevancy = apply_filters( 'eboywp_use_search_relevancy', true, $this );
        ...
        // Debug
        if ( 'on' == EWP()->helper->get_setting( 'debug_mode', 'off' ) ) {
        ...
        if ( 'wp' != $this->template['name'] ) {
            if ( ! $first_load || $is_bfcache || apply_filters( 'eboywp_template_force_load', false ) ) {
    ...
    function get_query_args() {
        ...
        if ( apply_filters( 'eboywp_template_use_archive', false ) ) {
    ...
    function get_filtered_post_ids() {
        ...
        $post_ids = apply_filters( 'eboywp_pre_filtered_post_ids', $post_ids, $this );

        // Determine whether we need to store unfiltered post IDs
        $store_ids = apply_filters( 'eboywp_store_unfiltered_post_ids', false );
        ...
        return apply_filters( 'eboywp_filtered_post_ids', $post_ids, $this );
    }
    ...
    function get_template_html() {
        ...
        $output = apply_filters( 'eboywp_template_html', false, $this );
    ...
    function get_result_count( $params = array() ) {
        ...
        return apply_filters( 'eboywp_result_count', $output, array(
            'lower' => $lower,
            'upper' => $upper,
            'total' => $total_rows,
        ) );
    ...
    function get_sort_options() {
        ...
        $options = array(
            'default' => array(
                'label' => __( 'Sort by', 'EWP' ),
                'query_args' => array()
            ),
            'title_asc' => array(
                'label' => __( 'Title (A-Z)', 'EWP' ),
                'query_args' => array(
                    'orderby' => 'title',
                    'order' => 'ASC',
                )
        ...
        return apply_filters( 'eboywp_sort_options', $options, array(
    ...
    function get_sort_html( $params = array() ) {
        ...
        if ( isset( $this->sort_options ) ) {
            $output = '<select class="eboywp-sort-select">';
        ...
        return apply_filters( 'eboywp_sort_html', $output, array(
    ...
    function paginate( $params = array() ) {
        ...
        if ( 1 < $total_pages ) {
            ...
            $text_page      = __( 'Page', 'EWP' );
            $text_of        = __( 'of', 'EWP' );

            // "Page 5 of 150"
            $output .= '<span class="eboywp-pager-label">' . "$text_page $page $text_of $total_pages</span>";

            if ( 3 < $page ) {
                $output .= '<a class="eboywp-page first-page" data-page="1">&lt;&lt;</a>';
            }
            if ( 1 < ( $page - 10 ) ) {
                $output .= '<a class="eboywp-page" data-page="' . ($page - 10) . '">' . ($page - 10) . '</a>';
            }
            for ( $i = 2; $i > 0; $i-- ) {
                if ( 0 < ( $page - $i ) ) {
                    $output .= '<a class="eboywp-page" data-page="' . ($page - $i) . '">' . ($page - $i) . '</a>';
                }
            }

            // Current page
            $output .= '<a class="eboywp-page active" data-page="' . $page . '">' . $page . '</a>';
            ...
            if ( $total_pages > ( $page + 2 ) ) {
                $output .= '<a class="eboywp-page last-page" data-page="' . $total_pages . '">&gt;&gt;</a>';
            }
        }

        return apply_filters( 'eboywp_pager_html', $output, array(
            'page' => $page,
            'per_page' => $per_page,
    ...
    function get_per_page_box() {
        $options = apply_filters( 'eboywp_per_page_options', array( 10, 25, 50, 100 ) );

        $output = '<select class="eboywp-per-page-select">';
        $output .= '<option value="">' . __( 'Per page', 'EWP' ) . '</option>';
        foreach ( $options as $option ) {
            $output .= '<option value="' . $option . '">' . $option . '</option>';
        }
        $output .= '</select>';

        return apply_filters( 'eboywp_per_page_html', $output, array(
            'options' => $options,
        ) );
    }
}
